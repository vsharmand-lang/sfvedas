...
class TutorialValidator:
    ...
    def _check_breadcrumbs(self):
        if '/tutorials/?tag=security-compliance' not in self.content:
            self.log_error("Breadcrumb backlink does not use expected absolute path '/tutorials/?tag=security-compliance'")
    ...
    def _check_spelling(self):
        ...
        for pattern, replacement in us_words.items():
            ...
            if matches:
                self.log_warning(f"Contains potential US spelling match for '{pattern}' (should be '{replacement}'). Found {len(matches)} occurrences.")
...
def main():
    ...
    sec_dirs = sorted([d for d in os.listdir(TUTORIALS_DIR) if d.startswith('sec-') and os.path.isdir(os.path.join(TUTORIALS_DIR, d))])
    
    print("============================================================")
    print(f"RUNNING AUTOMATED GRAMMAR TESTS ON {len(sec_dirs)} SEC TUTORIALS")
    print("============================================================")
    ...
    print("============================================================")
    if all_ok:
        print("SUCCESS: ALL SEC TUTORIALS COMPLIED WITH THE SITE GRAMMAR SPECIFICATIONS!")
        sys.exit(0)
    else:
        print("FAILURE: SOME SEC TUTORIALS HAVE COMPLIANCE ISSUES THAT NEED FIXING.")
        sys.exit(1)
