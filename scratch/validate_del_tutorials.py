...
class TutorialValidator:
    ...
    def _check_breadcrumbs(self):
        ...
        if '/tutorials/?tag=delivery-management' not in self.content:
            self.log_error("Breadcrumb backlink does not use expected absolute path '/tutorials/?tag=delivery-management'")
...
def main():
    ...
    del_dirs = sorted([d for d in os.listdir(TUTORIALS_DIR) if d.startswith('del-') and os.path.isdir(os.path.join(TUTORIALS_DIR, d))])
    
    print("============================================================")
    print(f"RUNNING AUTOMATED GRAMMAR TESTS ON {len(del_dirs)} DEL TUTORIALS")
    print("============================================================")
    ...
    print("============================================================")
    if all_ok:
        print("SUCCESS: ALL DEL TUTORIALS COMPLIED WITH THE SITE GRAMMAR SPECIFICATIONS!")
        sys.exit(0)
    else:
        print("FAILURE: SOME DEL TUTORIALS HAVE COMPLIANCE ISSUES THAT NEED FIXING.")
        sys.exit(1)
